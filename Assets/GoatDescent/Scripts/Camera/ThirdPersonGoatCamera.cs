...
namespace GoatDescent
{
    ...
    public sealed class ThirdPersonGoatCamera : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            if (!target) return;
            if (Cursor.lockState == CursorLockMode.Locked) { yaw += Input.GetAxis("Mouse X") * sensitivity * Time.deltaTime; pitch = Mathf.Clamp(pitch - Input.GetAxis("Mouse Y") * sensitivity * Time.deltaTime, minPitch, maxPitch); }
            Quaternion orbit = Quaternion.Euler(pitch, yaw, 0); Vector3 pivot = target.position + Vector3.up * height;
            ...
            if (Physics.SphereCast(pivot, collisionRadius, orbit * Vector3.back, out var hit, distance, worldMask, QueryTriggerInteraction.Ignore)) actualDistance = Mathf.Max(.55f, hit.distance - .08f);
