...
namespace GoatDescent
{
    public sealed class SlopeRun : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            if(help)GUI.Label(new Rect(35,103,380,75),"WASD — рулить • Space — короткий прыжок\nCtrl — тормозить • F — зацепиться за скалу\nR — снова на вершину",copy);
            GUI.Label(new Rect(23,height-33,width-46,25),"WASD — движение   •   Space — прыжок   •   Ctrl — тормоз   •   F — суперкопыта   •   R — вершина",copy);
            var grip=goat.GetComponent<GoatGripController>();
            if(grip)
            {
                float right=width-270f;
                GUI.color=new Color(.04f,.08f,.12f,.82f);GUI.DrawTexture(new Rect(right,20,250,158),Texture2D.whiteTexture);GUI.color=Color.white;
                GUI.Label(new Rect(right+14,30,225,25),grip.SuperActive?"ДЕРЖИМСЯ ЗА СКАЛУ":"СУПЕРКОПЫТА — F",copy);
                GUI.color=new Color(.2f,.27f,.31f);GUI.DrawTexture(new Rect(right+14,62,220,9),Texture2D.whiteTexture);
                GUI.color=new Color(.38f,.92f,.85f);GUI.DrawTexture(new Rect(right+14,62,220*grip.SuperCharge01,9),Texture2D.whiteTexture);GUI.color=Color.white;
                GUI.Label(new Rect(right+14,84,225,42),"Заряд восстанавливается на устойчивом камне.",copy);
                var balance=goat.GetComponent<GoatSlopeBalance>();
                if(balance)
                {
                    GUI.Label(new Rect(right+14,126,225,22),$"БАЛАНС • копыта {balance.HoovesHolding}/4",copy);
                    GUI.color=new Color(.2f,.27f,.31f);GUI.DrawTexture(new Rect(right+14,150,220,9),Texture2D.whiteTexture);
                    GUI.color=Color.Lerp(new Color(.94f,.33f,.23f),new Color(.46f,.82f,.55f),balance.Balance01);
                    GUI.DrawTexture(new Rect(right+14,150,220*balance.Balance01,9),Texture2D.whiteTexture);
