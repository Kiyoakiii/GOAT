...
namespace GoatDescent
{
    ...
    public sealed class GoatGripController : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (leftTrace) leftTrace.emitting = IsGripping;
            if (rightTrace) rightTrace.emitting = IsGripping;
        ...
        public void ReleaseForJump() { IsGripping = false; releasedUntil = Time.time + .32f; }
        ...
        private void FixedUpdate()
        {
            ...
            if (!body || body.isKinematic || (wall && wall.IsAiming)) { IsGripping = false; return; }
        ...
        private void OnDisable() { held = false; IsGripping = false; }
